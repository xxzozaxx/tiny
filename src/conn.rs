...
impl<'poll> Conn<'poll> {
    ...
    pub fn join(&mut self, chans: Vec<&str>) {
        self.status.get_stream_mut().map(|stream| {
            wire::join(stream, chans.as_slice()).unwrap();
        });
    ...
    pub fn part(&mut self, chan: &str) {
        ...
        self.auto_join.drain_filter(|c| c == chan);
    }
    ...
    fn handle_msg(&mut self, msg: Msg, evs: &mut Vec<ConnEv>, logger: &mut Logger) {
        ...
        } = msg
        {
            if let Some(mut stream) = self.status.get_stream_mut() {
                ...
                let chans: Vec<&str> = self.auto_join.iter().map(|s| s.as_str()).collect();
                wire::join(&mut stream, chans.as_slice()).unwrap();
