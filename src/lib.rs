                        ...
                        for conn_idx in 0..tiny.conns.len() {
                            {
                                ...
                                conn.tick(&mut conn_evs, &mut tiny.logger);
                            }
        ...
        for ev in evs.drain(..) {
            match self.tui.handle_input_event(ev) {
                ...
                TUIRet::Lines { lines, from } => {
                    self.logger
                        .debug(&format!("Input source: {:#?}, lines: {:?}", from, lines));
                    for line in lines {
                ...
                ev => {
                    self.logger.debug(&format!("Ignoring event: {:?}", ev));
                }
            }
        }
        abort
        ...
        match msg.cmd {
            Cmd::PRIVMSG {
                ...
                match target {
                    ...
                    wire::PrivMsgTarget::User(target) => {
            ...
            Cmd::JOIN { chan } => match pfx {
                ...
                pfx => {
                    self.logger
                        .debug(&format!("Weird JOIN message pfx {:?}", pfx));
                }
            },

            Cmd::PART { chan, .. } => match pfx {
                Some(Pfx::User { nick, .. }) => if nick != conn.get_nick() {
                    let serv_name = conn.get_serv_name();
                ...
                pfx => {
                    self.logger
                        .debug(&format!("Weird PART message pfx {:?}", pfx));
                }
            },
            ...
            Cmd::QUIT { .. } => match pfx {
                ...
                pfx => {
                    self.logger
                        .debug(&format!("Weird QUIT message pfx {:?}", pfx));
                }
            },
            ...
            Cmd::NICK { nick } => match pfx {
                ...
                pfx => {
                    self.logger
                        .debug(&format!("Weird NICK message pfx {:?}", pfx));
                }
            },
            ...
            Cmd::CAP {
                ...
                match subcommand.as_ref() {
                    ...
                    cmd => {
                        self.logger
                            .debug(&format!("CAP subcommand {} is not handled", cmd));
                    }
                };
            }
