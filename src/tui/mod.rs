...
impl TUI {
    ...
    pub fn switch(&mut self, string: &str) {
        ...
        for (tab_idx, tab) in self.tabs.iter().enumerate() {
            match tab.src {
                MsgSource::Serv { ref serv_name } => if serv_name.contains(string) {
                    next_idx = tab_idx;
                    break;
                },
                MsgSource::Chan { ref chan_name, .. } => if chan_name.contains(string) {
                    next_idx = tab_idx;
                    break;
                },
                MsgSource::User { ref nick, .. } => if nick.contains(string) {
                    next_idx = tab_idx;
                    break;
                },
            }
        ...
        F: Fn(&mut Tab, bool),
    {
        ...
        match *target {
            MsgTarget::Server { serv_name } => for (tab_idx, tab) in self.tabs.iter().enumerate() {
                if let MsgSource::Serv {
                    serv_name: ref serv_name_,
                } = tab.src
                {
