...
pub struct MessagingUI {
    ...
    current_nick: Option<Rc<RefCell<String>>>,
...
impl MessagingUI {
    ...
    pub fn set_nick(&mut self, nick: Rc<RefCell<String>>) {
        self.current_nick = Some(nick);
    }

    pub fn get_nick(&self) -> Option<Rc<RefCell<String>>> {
    ...
    pub fn draw(&self, tb: &mut Termbox, pos_x: i32, pos_y: i32) {
        ...
        if let &Some(ref rc) = &self.current_nick {
            if self.draw_current_nick {
                let nick_ref = rc.borrow();
                let nick: &str = nick_ref.as_str();

                use std::io::Write;
                use std::io;
                writeln!(io::stderr(), "nick: {}", nick);
